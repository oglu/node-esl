...
var Connection = exports.Connection = function() {
    ...
    //evens required for the module to operate properly
    ...
    //"Outbound" connection (coming from FSW)
        ...
        this.once('esl::event::CHANNEL_DATA::**', function(evt) {
            self.subscribe(self.reqEvents, function(e) {
                self.emit('esl::ready');
            });
        });
...
Connection.prototype.send = function(command, args, cmd_uuid) {
    var self = this;
    ...
    try {
        ...
        if(cmd_uuid) {
            self.socket.write("Event-UUID: "+ cmd_uuid +"\n");
        }
        if(args) {
...
Connection.prototype.bgapi = function(command, args, jobid, cb) {
    ...
    var self = this,
        params = {},
        addToFilter = removeFromFilter = function(cb) { if(cb) cb(); },
...
Connection.prototype.filter_delete = function(header, value, cb) {
...
Connection.prototype.execute = function(app, arg, uuid, cb) {
    ...
    if(self._inbound) {
        ...
        if(!uuid) {
            self.api('create_uuid', function(evt) {
                uuid = evt.getBody();
                self._doExec(uuid, 'execute', opts, cb, uuidgen.v1());
            });
        }
    ...
    this.bgapi('show ' + item + ' as ' + format, function(e) {
        ...
        default: //delim seperated values, custom parsing
            if(format.indexOf('delim')) {
                ...
                parsed = { row_count: lines.length - 1, rows: [] };
...
Connection.prototype._doExec = function(conuuid, cmd, args, cb, cmd_uuid) {
    ...
    this.once('esl::event::CHANNEL_EXECUTE_COMPLETE::' + cmd_uuid, cb);

    this.send('sendmsg ' + conuuid, args, cmd_uuid);
};
...
Connection.prototype._onEvent = function(event, headers, body) {
    var emit = 'esl::event',
    uuid = event.getHeader('Job-UUID') || event.getHeader('Application-UUID') || event.getHeader('Unique-ID') || event.getHeader('Core-UUID');
